...
const ENCLAVE_ID_PRIVATE_KEY_PATH = '/app/id.sec';
...
app.get('/api/trades/:id', (req, res) => {
  ...
  // Find the trade in either pending or history
  const trade = [...pendingTrades, ...tradeHistory].find(
    trade => trade.id === id
  );
...
app.post('/api/trades/:id/confirm', (req, res) => {
  ...
  // Find the trade in pending
  const tradeIndex = pendingTrades.findIndex(
    trade => trade.id === id
  );
  ...
  // Update trade status
  pendingTrades[tradeIndex].status = 'confirmed';
  pendingTrades[tradeIndex].confirmationTimestamp = new Date().toISOString();
  
  // Move to history
  const confirmedTrade = pendingTrades[tradeIndex];
  tradeHistory.push(confirmedTrade);
...
app.post('/api/trades/:id/cancel', (req, res) => {
  ...
  // Update trade status
  pendingTrades[tradeIndex].status = 'cancelled';
  pendingTrades[tradeIndex].cancellationTimestamp = new Date().toISOString();
  
  // Move to history
  const cancelledTrade = pendingTrades[tradeIndex];
  tradeHistory.push(cancelledTrade);
  pendingTrades.splice(tradeIndex, 1);
