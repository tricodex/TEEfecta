...
if [ -z "$MARLIN" ]; then
  echo "MARLIN environment variable not set, using default wallet"
  WALLET_KEY="$MARLIN"
else
  WALLET_KEY="$MARLIN"
fi
...
echo "Running: NETWORK=arbitrum oyster-cvm deploy --wallet-private-key \"$WALLET_KEY\" --docker-compose minimal-docker-compose.yml --duration-in-minutes 60"

# Execute the deployment command
DEPLOY_OUTPUT=$(NETWORK=arbitrum oyster-cvm deploy --wallet-private-key "$WALLET_KEY" --docker-compose minimal-docker-compose.yml --duration-in-minutes 60)
...
while [ $ATTEMPT -le $MAX_ATTEMPTS ]; do
  ...
  # Get the job status
  JOB_STATUS=$(NETWORK=arbitrum oyster-cvm list --address $(echo $WALLET_KEY | oyster-cvm wallet-address))
